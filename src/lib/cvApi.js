...
export const cvLanguageLabels = {
  en: 'English',
  de: 'Deutsch',
};

export async function validateCvCode(code, signal) {
...
export async function requestCvAccess(email) {
  if (!blogApiBaseUrl) {
    throw new Error('The CV API is not configured.');
  }
...
export function buildCvDownloadUrl(language, code) {
  const normalizedCode = String(code || '').trim();
